...
function renderConfirmation(userDetails) {
  ...
  const thankYouMessage = `Thank you ${userDetailsParams.get(
    "firstName"
  )} ${userDetailsParams.get("lastName")} for subscribing!`;

  const planDetails = `Your ${userDetailsParams.get(
    "subscriptionPlans"
  )} plan free trial will expire after 30 days. `;

  const freebieDetails = `${userDetailsParams.get(
    "freeAlbums"
  )} CD album will be delivered to your registered address within 3 business days. `;
  const emailMessage = `Complete details will be sent to ${userDetailsParams.get(
    "email"
  )} within 5 minutes.`;
